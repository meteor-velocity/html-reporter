...
Template.velocityReports.helpers({
  ...
  noFrameworkFiles: function () {
    var frameworkResult = VelocityAggregateReports.findOne({name: this.name});
    if (frameworkResult && frameworkResult.result === 'completed'){
      if (! VelocityTestFiles.findOne({targetFramework: this.name}))
        return true
    }
    return false;
  },
...
Template.velocityControlPanel.helpers({
  ...
  mochaPresent: function () {
    //XXX hard-coding mocha iframe support for now
    return !! VelocityAggregateReports.findOne({'name': 'mocha-web'});
  },
